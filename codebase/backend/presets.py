...
golden_set_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "eval", "golden_set.json")
try:
    with open(golden_set_path, "r", encoding="utf-8") as f:
        golden_data = json.load(f)
    
    sentences = []
    for i, case in enumerate(golden_data.get("cases", [])):
        sentences.append({
            "id": i + 1,
            ...
            "text": case.get("sentence"),
            "type": f"{case.get('category')} ({case.get('gold_label')})",
            "color": "gray",
            "gold_span": case.get("gold_span") or "",
            "gold_reason": case.get("gold_reason") or "",
            "gold_suggest": case.get("gold_suggestion") or "",
            "ai_label": None,
            "ai_issue": None,
            "criteria": None,
            "notes": "Chưa kiểm thử (Bấm 'Chạy đánh giá' để bắt đầu)"
        })
        
    PRESETS[6] = {
        "id": 6,
        "title": "🎯 Golden Set Evaluation — 21 Test Cases (Dynamically Loaded)",
        "meta": "21 cases · Ground Truth chuẩn con người · Trạng thái: Chưa đánh giá",
        "description": "Bộ đánh giá 21 ca kiểm thử đối chiếu với Ground Truth của chuyên gia sư phạm (Chưa chạy kiểm thử).",
        "sentences": sentences
    }
except Exception as e:
    print(f"Failed to load dynamic golden_set.json: {e}")
